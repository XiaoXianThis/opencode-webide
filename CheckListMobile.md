...
- [ ] 定义五个 Tab 各自的二级页面与抽屉/模态层级
- [ ] 定义桌面/移动布局切换策略（媒体查询 + Route guard，桌面命中 `/m/*` 且非移动设备时回退 `/`）

## 阶段 2：移动端 App Shell 与路由

### 路由统一接管（前置）
- [ ] 安装 `react-router-dom`
- [ ] 用 `BrowserRouter` 包裹 `App`，定义根 `<Routes>`：`/`（桌面）、`/login`、`/m/*`（移动）
- [ ] **删除** `App.tsx` 内现有的 `pathname` state、`popstate`/`webide:navigate` 监听
- [ ] 把 `Login` 改造为路由组件，登录成功后 `navigate('/m')` 或 `'/'`（按 `useIsMobile`）
- [ ] 全局搜索 `webide:navigate` / `window.location.pathname` 等用法并替换为 `useNavigate` / `useLocation`
- [ ] 服务端 SPA fallback：`index.ts` / `proxy.ts` 对非 `/api/*` 的 GET 全部回退 `index.html`（确保深链可用）
- [ ] 路由错误边界 `<ErrorBoundary>` + `NotFound` 兜底

### 移动 App Shell
- [ ] 设计路由表并新增 `MobileRoutes`，挂载在 `/m/*`
- [ ] 新增 `useIsMobile()` Hook（`matchMedia('(max-width: 767px)')`，订阅变化）
- [ ] 根路由根据 `useIsMobile()` 自动重定向：桌面访问 `/m/*` 跳 `/`，移动访问 `/` 跳 `/m`
- [ ] 新增 `MobileAppShell`（Outlet + 顶栏 + 底部导航 + safe-area 包裹）
- [ ] 新增 `MobileBottomNav`，使用 `NavLink` 高亮当前 Tab
- [ ] 新增 `MobileTopBar`：当前项目名 + 二级页返回按钮 + 抽屉触发
- [ ] 保留桌面 `WorkspaceCenter + ChatPanel` 布局不变
- [ ] 处理 `100dvh`（fallback `100vh`）高度
- [ ] 适配 iOS safe-area：`env(safe-area-inset-top/bottom)`
- [ ] 处理软键盘弹出时 `visualViewport` 高度变化
- [ ] 禁止主体横向溢出（`overflow-x: hidden`）
- [ ] 为内容容器预留底部 Tab 高度的 padding
- [ ] 增加 App Shell 单元测试（路由匹配、Tab 切换、safe-area class、移动/桌面互跳）

## 阶段 3：项目列表 Tab（`/m/projects`）

- [ ] 抽取项目展示逻辑，复用 `useProjectsStore`
- [ ] 新增 `MobileProjectsPage`：项目卡片列表（项目名 / 路径 / 当前态徽标）
- [ ] 支持点击切换当前项目，切换后跳转 `/m/chat` 或保持当前 Tab
- [ ] 切换项目后刷新文件树、会话、PTY 状态
...
- [ ] 新增 `MobileChatPage`：默认渲染当前会话 `ChatBody`
- [ ] **左侧抽屉**：滑动或点击顶栏图标打开 `SessionList`，点击会话 → 路由跳 `/m/chat/:id`
- [ ] 抽屉支持搜索会话、新建会话、删除会话
- [ ] 保留消息搜索、停止生成、总结、权限提醒入口（顶栏菜单收纳）
- [ ] 移动端 Composer：自适应高度 + 软键盘弹出时滚动到底部
- [ ] 处理 `visualViewport` 变化导致的输入框遮挡
- [ ] 消息列表自动滚动 + 用户上滑时暂停自动滚动
- [ ] 大消息列表虚拟化（如已有则复用）
...
- [ ] 新增 `MobileFilesPage`：当前目录列表 + 面包屑
- [ ] 支持目录进入/返回上级（路由 `/m/files/*path` 驱动）
- [ ] 文件状态徽标（git 状态、未保存等）
...
- [ ] 点击文件 → 进入文件查看/编辑视图（移动端独立页 `/m/files/*path`，桌面端 `EditorPane`）
...
- [ ] 新增 `MobileTerminalPage`：默认渲染当前 PTY，无则显示空态 + 新建按钮
- [ ] 会话列表抽屉（顶栏图标触发）：列表、新建、切换、关闭
- [ ] 路由切换 PTY：`/m/terminal/:ptyId`
...
- [ ] 新增 `MobileMePage`：分组列表（iOS 设置风格）
- [ ] **主题切换**：深色 / 浅色 / 跟随系统
- [ ] **模型 / Provider 选择**：列表 + 默认模型设置
- [ ] **权限中心**：工具/命令权限管理
- [ ] **连接状态**：服务地址、健康状态、版本号、构建时间
- [ ] **PWA 安装引导**：检测 `beforeinstallprompt`；iOS 显示手动添加到主屏教程
- [ ] **账户信息 + 退出登录**：当前用户、Token 状态、退出确认
- [ ] 二级页面均使用路由（便于返回手势/浏览器后退）
- [ ] 设置项单元测试
...
- [ ] 配置 `name=opencode WebIDE`、`short_name=opencode`、`description`
- [ ] **`start_url=/m`**、**`scope=/`**、**`display=standalone`**、`orientation=portrait`
- [ ] 配置 `theme_color`、`background_color`（与深色主题对齐）
- [ ] 准备 192x192、512x512、512x512 maskable 三套 icon（放在 `public/icons/`）
- [ ] 在 `index.html` 引入 `<link rel="manifest">`
- [ ] 添加 iOS PWA meta：`apple-mobile-web-app-capable`、`apple-mobile-web-app-status-bar-style`、`apple-mobile-web-app-title`
- [ ] 添加 Apple touch icon（180x180）
- [ ] 添加 iOS 启动 splash（按主流尺寸，至少 1 张）

### Service Worker（workbox-build）
- [ ] 安装 `workbox-build`（devDependency）
- [ ] 新增 `scripts/build-sw.ts`：调用 `injectManifest` 或 `generateSW` 生成 `dist/public/sw.js`
- [ ] `package.json` `build` 脚本串接：先 `bun build` 再 `bun scripts/build-sw.ts`
- [ ] 新增 `src/client/sw/sw.ts` 源文件（injectManifest 模式）：precache app shell + 静态资源
- [ ] runtime 缓存策略：`StaleWhileRevalidate` 对静态资源、`NetworkFirst` 对 HTML（含 `/m/*` 深链）
- [ ] **bypass 列表**：`/api/event*`、`/api/pty*`、`/api/permission*`、`/api/auth*`、`/api/file/write*`、所有 SSE 请求（`accept: text/event-stream`）
- [ ] 离线 fallback：`/offline.html` 仅显示静态壳 + “重新连接”按钮
- [ ] 在 `main.tsx` 注册 SW（仅 production）
- [ ] SW 升级策略：`skipWaiting` + `clientsClaim` + 顶部条提示“新版本可用，点击刷新”

### 安装与在线状态
- [ ] `useInstallPrompt()` Hook 捕获 `beforeinstallprompt`（Android/桌面 Chrome）
- [ ] 我的 Tab 显示安装按钮（可用时）；iOS 显示“添加到主屏幕”图文教程
- [ ] `useOnlineStatus()` Hook 监听 `online`/`offline`
- [ ] 顶栏出现“已离线，仅可浏览缓存内容”醒目条
- [ ] **离线状态下所有写操作 UI 置灰 + 提示**
...
- [ ] 增加终端 Tab 测试
- [ ] 增加设置 Tab 测试
- [ ] 增加 PWA manifest 测试或构建校验
- [ ] 使用 Playwright 跑移动端 viewport E2E
- [ ] 验证 iPhone 尺寸布局
- [ ] 验证 Android 手机尺寸布局
- [ ] 验证横屏布局不崩
- [ ] 验证桌面端布局无回归
