...
function flattenSessions(
  ...
  const visit = (session: Session, depth: number) => {
    ...
    for (const child of childrenByParent[session.id] ?? [])
      visit(child, depth + 1);
  };
  for (const session of sessions.filter((item) => !item.parentID))
    visit(session, 0);
...
export function SessionSidebar({ embedded = false }: { embedded?: boolean }) {
  ...
  const rows = useMemo(
    () =>
      searchQuery.trim()
        ...
        : flattenSessions(sessions, childrenByParent),
  ...
  return (
    <aside
      className={
        embedded
          ? "flex h-full min-h-0 min-w-0 flex-col bg-content1"
          : "flex h-full w-64 shrink-0 flex-col border-r border-default-200 bg-content1"
      }
    >
          ...
          >
            {rows.map(({ session, depth }) => (
              <ListBox.Item
                key={session.id}
                id={session.id}
                textValue={session.title || session.id}
              >
                <div
                ...
                >
                  {editingId === session.id ? (
                    <Input
                      autoFocus
                      aria-label={`编辑会话标题 ${session.title || session.id}`}
                      size="sm"
                      value={draft}
                      onChange={(event) => setDraft(event.target.value)}
                      onBlur={() => void saveTitle(session.id)}
                      onKeyDown={(event) => {
                        if (event.key === "Enter") void saveTitle(session.id);
                        if (event.key === "Escape") setEditingId(null);
                      }}
                    />
                  ) : (
                    <Label
                      aria-label={`加载子会话 ${session.title || session.id}`}
                  ...
                  <Popover.Content className="w-40 p-1">
                    <div className="flex flex-col gap-1">
                      <Button
                        ...
                        aria-label={`分叉会话 ${session.title || session.id}`}
                        onPress={() => void fork(session.id)}
                      >
                      ...
                      {session.share ? (
                        <Button
                          ...
                          variant="light"
                          className="justify-start"
                          aria-label={`取消分享 ${session.title || session.id}`}
                          onPress={() => void unshare(session.id)}
                        >
                        ...
                        <Button
                          ...
                          variant="light"
                          className="justify-start"
                          aria-label={`分享会话 ${session.title || session.id}`}
                          onPress={() => void share(session.id)}
                        >
